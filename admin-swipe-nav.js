(() => {
  ...
  function isInteractive(target) {
    return Boolean(target?.closest?.(
      "input,textarea,select,button,a,label,[contenteditable='true'],.admin-tabs,.editor-stage,.editor-controls,.image-picker,.promo-v2-actions,.product-actions"
    ));
  }
  ...
  function currentPanel() {
    return root.querySelector(".tab-panel.active");
  }
  ...
  function switchBySwipe(dx) {
    ...
    if (!panel || index < 0 || nextIndex < 0 || nextIndex >= buttons.length) {
      animateBack(panel);
      return;
    }
