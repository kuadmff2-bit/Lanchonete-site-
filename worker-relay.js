...
async function authorizeAdmin(request, env, ctx) {
  const authUrl = new URL("/api/auth", request.url);
  return appWorker.fetch(new Request(authUrl, { method: "GET", headers: request.headers }), env, ctx);
}

function authHeaders(authResponse) {
...
async function handleRobotConnectionV2(request, env, ctx) {
  ...
  if (!service) {
    return json({ configured: false, connected: false, qrReady: false, pairingCode: null, authState: "not_configured", message: "O serviço 24 horas do WhatsApp ainda não foi vinculado a este sistema." }, 200, responseHeaders);
  }

  let path = "/control/status";
  let method = "GET";
  let body = null;
  ...
  if (request.method === "POST") {
    ...
    if (payload?.action === "pair-code") {
      path = "/control/pair-code";
      method = "POST";
      body = JSON.stringify({ phoneNumber: String(payload?.phoneNumber || "") });
      ...
      return json({ error: "Ação inválida." }, 400, responseHeaders);
    }
  ...
  try {
    const remote = await fetch(`${service.url}${path}`, {
      method,
      headers: {
    ...
    if (!remote.ok) {
      return json({ configured: true, connected: false, qrReady: false, pairingCode: null, authState: "unreachable", error: String(data?.error || `WhatsApp indisponível (${remote.status}).`).slice(0, 300) }, 502, responseHeaders);
    }

    if (request.method === "POST") {
      return json({ configured: true, ok: true, accepted: true, action: data?.action || null, message: data?.message || "Solicitação enviada ao WhatsApp." }, 202, responseHeaders);
    }
    ...
    return json({
      ...
      authState: String(data?.authState || "unknown").slice(0, 80),
      connectedAt: data?.connectedAt || null,
      lastError: data?.lastError || null
    }, 200, responseHeaders);
  } catch {
    return json({ configured: true, connected: false, qrReady: false, pairingCode: null, authState: "unreachable", error: "Não foi possível falar com o serviço 24 horas do WhatsApp." }, 502, responseHeaders);
  }
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/robot/connection") {
      return handleRobotConnectionV2(request, runtimeEnv, ctx);
    }
