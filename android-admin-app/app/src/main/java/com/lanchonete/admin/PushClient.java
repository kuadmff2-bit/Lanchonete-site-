...
public final class PushClient {
    ...
    private static final String APP_USER_AGENT = "LanchoneteAdminApp/1.9";
        ...
        new Thread(() -> {
            ...
            try {
                ...
                if (code >= 200 && code < 300) {
                    ...
                    if (registered) {
                        Log.i(TAG, serverConfigured
                                ? "Aparelho registrado e servidor de notificações pronto."
                                : "Aparelho registrado, mas o servidor informou push incompleto.");
                    } else {
                        Log.w(TAG, "Servidor respondeu sem confirmar o registro do aparelho.");
                    }
                } else {
                    preferences.edit()
                            .putBoolean("server_registered", false)
                            .apply();
    ...
    public static boolean isServerConfigured(Context context) {
        return context.getSharedPreferences("push", Context.MODE_PRIVATE)
                .getBoolean("server_configured", false);
    }
