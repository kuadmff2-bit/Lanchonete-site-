(() => {
  ...
  function installStyles() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      /* WhatsApp: deixa só o que é necessário para conectar e controlar a sessão. */
      ...
      /* Cadastro de promoção mais limpo. */
      ...
      /* Os antigos checkboxes viram controles grandes e claros. */
      ...
      body.admin-view .promo-v2-toggles label:has(input:checked){
        border-color:color-mix(in srgb,var(--brand-primary,var(--orange)) 62%,var(--line))!important;
        background:color-mix(in srgb,var(--brand-primary,var(--orange)) 10%,var(--card2,var(--surface2)))!important;
      }

      /* Botões de salvar/cancelar/imagem mais fáceis de tocar no celular. */
  ...
  function cleanupPromo() {
    ...
    const visibleLabel = document.getElementById("promoV2Visible")?.closest("label");
    const availableLabel = document.getElementById("promoV2Available")?.closest("label");
    if (visibleLabel) visibleLabel.lastChild.textContent = " Mostrar no cardápio";
    if (availableLabel) availableLabel.lastChild.textContent = " Disponível para pedido";
  }
