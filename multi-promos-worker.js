...
export async function readPromotions(env) {
  ...
  if (raw) {
    try {
      ...
      if (Array.isArray(parsed)) return parsed.map(normalizePromotion).filter((promo) => promo.title).slice(0, MAX_PROMOTIONS);
    } catch {}
  }
...
async function authorizeAdmin(request, env, ctx, appWorker) {
  ...
  const authResponse = await appWorker.fetch(new Request(url, {
    method: "GET",
...
export async function handlePromotionsApi(request, env, ctx, appWorker) {
  if (request.method === "GET") {
    return json({
      promotions: await readPromotions(env),
  ...
  catch { return json({ error: "Dados de promoções inválidos." }, 400, authHeaders(auth)); }
...
function addOrderToStats(stats, order) {
  if (stats.currentDate !== order.localDate) {
    stats.currentDate = order.localDate;
    stats.todayOrders = 0;
    stats.todayValue = 0;
  }
...
export async function createOrderWithPromotion(request, env, ctx, appWorker, suppliedBody = null) {
  ...
  if (!body) {
    try { body = await request.json(); }
    catch { return json({ error: "Dados do pedido inválidos." }, 400); }
  }
  ...
  if (!promo || !promo.active || !promo.orderEnabled || promoPrice <= 0) {
    return json({ error: "Essa promoção mudou ou não está mais disponível. Atualize o cardápio e tente novamente." }, 409);
  }
