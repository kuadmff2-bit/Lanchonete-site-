  ...
  function renderPanel() {
    ...
    host.innerHTML = `
      <section class="robot-admin-card robot-connection-card">
        <div class="robot-admin-header"><div><h2>Conexão do WhatsApp</h2><p>Esta conexão serve somente para enviar pedidos e avisos automáticos. Ela não atende nem responde clientes.</p></div><span class="robot-connection-badge is-preparing" id="robotConnectionBadge">Verificando</span></div>
        ...
        <div class="robot-actions"><button type="button" class="admin-secondary" id="robotConnectionRefresh">Atualizar estado</button><button type="button" class="admin-primary" id="robotConnectionReset">Conectar WhatsApp</button></div>
    ...
    host.querySelector("#robotConnectionReset").addEventListener("click", async () => {
      ...
      try {
        ...
        setTimeout(() => { delete status.dataset.locked; loadConnection(host); }, 1400);
      } catch (error) {
